...
export default function Home() {
  ...
  const getProduct = async () => {
    const response = await axios.post(
      "https://foodpoint-backend-je3y.onrender.com/api/product/resturent",
      { restaurant_id: restrodata._id }
    );
        ...
        <div className="col mb-5 r mb-5 " style={{ marginTop: "100px" }}>
          ...
          <div className="row  d-flex align-items-center justify-content-between">
            <div className="col-sm-6 px-md-5 px-5 ">
              <p className="fs-2 fw-bold text-secondary mb-5">
                Welcom to{" "}
                <span className="text-warning border-bottom border-2 border-secondary text-uppercase">
                  {restrodata.name}
                </span>
              </p>
              <div className="fw-medium">
                <p className="font-monospace">
                  Thank you for joining our online delivery platform! We're
                  excited to partner with you and look forward to driving more
                  customers to your delicious dishes.
                </p>
                <p className="mb-0 text-secondary">
                  <i className="bi bi-star text-dark"> </i>{" "}
                  {`${restrodata.rating}`.slice(0, 4)}/5 Overall Ratings
                </p>
                <p className="mb-0 text-secondary">
                  <i className="bi bi-clock text-dark"> </i>{" "}
                  {restrodata?.timing?.openAt} to {restrodata?.timing?.closeAt}
                </p>
                <p className="mb-0 text-secondary">
                  <i className="bi bi-geo-alt text-dark"> </i>{" "}
                  {restrodata?.address?.street}, {restrodata?.address?.area}
                </p>
                <p className="mb-0 text-secondary">
            ...
            <div className="col-sm-6  p-0">
              <div className="carouselStyle">
                <button
                  onClick={prevImage}
                  className="arrowStyleL btn btn-outline-light"
                >
                  &lt;
                </button>
                <img
                  ...
                  alt="carousel"
                  className={`imageStyle w-75 rounded rounded-3 box-shadow ${
                    isTransitioning ? "carousel-image-exit" : ""
                  }`} // Add your transitioning class here
                />
                <button
                  onClick={nextImage}
                  className="arrowStyleR btn btn-outline-light"
                >
                  &gt;
                </button>
              </div>
          ...
          <div className="container-md px-5 mt-5 ">
            <div className="row px-md-5 ">
              <p className="fs-2 d-none d-sm-block fw-bold text-secondary text-center mb-5">
                <i className="bi bi-star pe-2"></i> Your top rated Products{" "}
                <i className="bi bi-star ps-2"></i>
              </p>
              <p className="fs-6 d-sm-none d-block fw-bold text-secondary text-center mb-5">
                <i className="bi bi-star pe-2"></i>Top rated Products
                <i className="bi bi-star ps-2"></i>
              </p>
              ...
              {productDetail.map((item, index) => {
                return (
                  <div key={index} className="col-sm-6 px-5-md mb-5">
                      ...
                      <div className="row m-0">
                        ...
                        <div className="col-4 ">
                          <img
                            ...
                            className="img-fluid rounded rounded box-shadow"
                            alt=""
                            style={{
                              height: "100px",
                              width: "100%",
