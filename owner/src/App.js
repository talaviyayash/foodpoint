...
function App() {
  ...
  const isApproved = useSelector(
    (state) => state.restaurant.RestaurantInfo.isApproved
  );
  // console.log(isApproved)
  const isVerified = useSelector(
    (state) => state.restaurant?.RestaurantInfo.isVerified
  );
  // console.log(isVerified)
  const refresh = async () => {
    try {
      const response = await axios.post(
        "https://foodpoint-backend-je3y.onrender.com/api/Restaurant/refresh",
        {},
