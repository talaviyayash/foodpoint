...
export default function OtpVerification() {
  ...
  const DeliveryBoyId = useSelector(
  ...
  const handelClick = async () => {
    ...
    const response = await axios.post(
      "https://foodpoint-backend-je3y.onrender.com/api/delivery/otpverify",
      {
        DeliveryBoyId,
        otp: otpSend,
      }
  ...
  const otpGenerate = async () => {
    const response = await axios.post(
      "https://foodpoint-backend-je3y.onrender.com/api/delivery/otpGenerate",
      { DeliveryBoyId }
    );
    console.log(response.data);
  };
  ...
  return (
    <>
      ...
      <nav className="">
        <div
          className="row p-0 m-0 d-flex align-items-center navbar fw-medium w-100 position-fixed box-shadow "
          style={{ height: "70px" }}
        >
          <div className="col d-flex align-items-center">
            <div className="me-auto">
              <p className="m-0 mx-3">
                <img
                  src="https://www.svgrepo.com/show/251521/food-pin.svg"
                  className="img-fluid main-logo me-2"
                  alt=""
                />
      ...
      <div className="container-fuild  d-flex justify-content-center align-items-center pt-sm-5">
        <div
          className="border p-3 py-4 mb-5 rounded box-shadow"
          style={{ width: "450px", marginTop: "65px" }}
        >
          <p className="fs-3 mb-3 fw-bold">OTP verification</p>
          <p className="text-secondary">
            Please enter the OTP &#40;One time password&#41; sent to your
            register Email Id to complete the verification
          </p>
          ...
          <div className="row m-0 ">
            ...
            <div className="d-flex">
              <small className="ms-auto mt-3" onClick={otpGenerate}>
                Didn't get code? <span className="text-primary">Resend</span>
              </small>
            </div>
          </div>
          ...
          <button
            className="btn btn-primary rounded rounded-3 py-2"
            onClick={() => {
              handelClick();
            }}
            disabled={
          ...
          <button
            className="ms-2 btn btn-outline-secondary rounded rounded-3 py-2"
            onClick={() => {
              navigate("/");
            }}
          >
