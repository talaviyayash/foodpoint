...
export default function PendingBoys() {
  ...
  const allDeliveryBoy = async () => {
    const response = await axios.get(
      "https://foodpoint-backend-je3y.onrender.com/api/delivery/allfetch"
    );
  ...
  const approveDeliveryBoy = async (id) => {
    ...
    const response = await axios.post(
      "https://foodpoint-backend-je3y.onrender.com/api/delivery/approvel",
      data
    );
    allDeliveryBoy();
  };
  const deleteDeliveryBoy = async (id) => {
    ...
    const responsxe = await axios.post(
      "https://foodpoint-backend-je3y.onrender.com/api/delivery/delete",
      data
    );
    allDeliveryBoy();
  };
          ...
          <div className="card-delivery text-start mb-5 box-shadow" key={index}>
            <div className="d-flex">
              <img
                ...
                className="img-logo-delivery ms-1"
                alt=""
              />
            ...
            <div className="d-flex justify-content-end mt-3">
              <button
                className="btn btn-sm btn-outline-success me-2 bg-light"
                onClick={() =>
                  setApproveModal({ isvisible: true, id: item._id })
                }
              >
                <i className="bi bi-check-circle"></i>
              </button>
              <button
                className="btn btn-sm btn-outline-danger me-3 bg-light"
                onClick={() =>
                  setDeleteModal({ isvisible: true, id: item._id })
                }
              >
        ...
        <div className="modal-content bg-light p-4 box-shadow">
          ...
          <div className="modal-actions d-flex ms-auto mt-auto">
            ...
            <button
              ...
              onClick={() => {
                approveDeliveryBoy(approveModal.id);
                setApproveModal({ isvisible: false, id: null });
              }}
            >
              Ok
            </button>
      ...
      <div className="modal-overlay" hidden={!deleteModal.isvisible}>
        <div className="modal-content bg-light p-4 box-shadow">
          <div className="text-center">
            ...
            <p className="fs-3 fw-bold mt-2">Are you sure?</p>
            <p className="font-light-thick">
              You want to Delete this Delivery Boy ?
            </p>
          </div>
          <div className="modal-actions d-flex ms-auto mt-auto">
            <button
            ...
            <button
              className="btn btn-danger px-3"
              onClick={() => {
                deleteDeliveryBoy(deleteModal.id);
                setDeleteModal({ isvisible: false, id: null });
              }}
            >
              Ok
