...
export default function YourProducts() {
  ...
  const getCategory = async () => {
    try {
      const response = await axios.post(
        "https://foodpoint-backend-je3y.onrender.com/api/category/fetchid",
        data
      );
  ...
  const chageCategory = async (id) => {
    ...
    try {
      const response = await axios.post(
        "https://foodpoint-backend-je3y.onrender.com/api/product/catagory",
        categoryId
      );
  ...
  const UpdateProduct = async () => {
    ...
    try {
      await axios.post(
        "https://foodpoint-backend-je3y.onrender.com/api/product/update",
        data
      );
    } catch (e) {
      console.log(e);
  ...
  const deleteProductById = async () => {
    await axios.post(
      "https://foodpoint-backend-je3y.onrender.com/api/product/delete",
  ...
  return (
    <>
      ...
      <div className="row m-0 mb-5">
        <h2 className="mt-5 text-center pt-5">
          <span className="border-bottom border-5">LIST OF PRODUCTS</span>
        </h2>

        <div className="row m-0 mt-5 mb-5">
          <div className="col-md-3 col-12  px-3">
            <div className="px-md-4 d-flex d-md-block">
              <p className="fs-5 py-2 px-2 fw-medium border border-2 border-dark rounded rounded-4 me-2">
                Categories
              </p>
              ...
              {category.map((item, index) => {
                return (
                  <p
                    className="py-2 ps-md-3 px-2 ms-1 fw-medium border rounded rounded-4 d-flex align-items-center"
                    style={{ backgroundColor: "rgb(226, 232, 240)" }}
                    key={index}
                    onClick={() => {
                      setCatagoryUpdateId({ _id: item._id });
            ...
            <div className="row p-0 d-flex justify-content-evenly">
              ...
              {product.map((item, index) => {
                return (
                  <div
                    key={index}
                    className="col-lg-3 col-md-4 col-sm-5 col-12 p-0  rounded rounded-5 category-cards"
                  >
                    <img
                      src={
                        item?.img ||
                      ...
                      className="item-img rounded rounded-5"
                      alt=""
                    />

                    <div className="items-details p-3">
                      <div
                        className="p-3 border rounded rounded-5 box-shadow"
                        style={{ backgroundColor: "white" }}
                      >
                        <small className="font-light-thick">
                          &#x2022; {categoryName || "Street Food"}
                        </small>
                        <p className="mb-0 text-success text-uppercase">
                          {item.name}
                        </p>
            ...
            <div className="modal-actions d-flex ms-auto mt-auto">
              ...
              <button
                onClick={() => {
                  setDeleteModal(false);
                  deleteProductById();
                  setCatagoryUpdateId({ ...catagoryUpdateId });
                }}
        ...
        <div className="modal-overlay" hidden={!editModal}>
          <div className="model-content-edit bg-light px-4 py-3 box-shadow rounded">
            <p className="fs-5 fw-bold d-flex border-bottom border-2 pb-2 align-items-center">
              Update Product{" "}
              <button
            ...
            <div>
              ...
              <input
                ...
                className="w-100 border py-1 px-3 rounded"
              ...
              <textarea
                ...
                className="w-100 py-1 px-3 border rounded "
