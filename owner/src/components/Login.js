  ...
  const getdata = async () => {
    try {
      const response = await axios.post(
        "https://foodpoint-backend-je3y.onrender.com/api/Restaurant/signin",
        data,
      ...
      if (response.data.login === true) {
        if (!response.data.RestaurantInfo.bankDetail) {
          ...
          navigate("RestroDetails", {
            state: { id: response.data.RestaurantInfo._id },
          });
        } else {
          dispatch(setRestroDetails(response.data));
          navigate("/Home");
  ...
  return (
    <div className="login-page  d-flex">
      <div className="login-container px-5 py-3 border rounded rounded-3">
        <h2 className="fs-2 text-center fw-bold text-secondary">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="w-100 mt-3 mb-2 btn btn-outline-danger"
          >
            Log In
          </button>
          <small className="">
