...
export default function OrderList() {
  ...
  const delivery_id = useSelector(
    (state) => state?.deliver?.deliverInfo?.deliveryBoyInfo?._id
  );
  const navigate = useNavigate();
  const processOrder = async () => {
    const response = await axios.get(
      "https://foodpoint-backend-je3y.onrender.com/api/order/orderpending"
    );
  ...
  const acceptOrder = async (id) => {
    ...
    const response = await axios.post(
      "https://foodpoint-backend-je3y.onrender.com/api/order/acceptOrder",
      {
        Order_id: id,
        delivery_id,
        status: "on the way",
  ...
  return (
    <>
      ...
      <div className="row m-0 p-3">
        <div
          className="col p-3 border box-shadow"
          style={{ marginTop: "100px", marginBottom: "50vh" }}
        >
