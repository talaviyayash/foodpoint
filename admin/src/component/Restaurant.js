  ...
  const getAllDetail = async () => {
    try {
      const response = await axios.get(
        "https://foodpoint-backend-je3y.onrender.com/api/Restaurant/fetchall"
      );
                ...
                <div className="mt-4 p-3">
                  <div className="row mb-5">
                    ...
                    {restaurantDetails.map((item, index) => {
                      return (
                        <div
                          className="restro rounded box-shadow mb-5 p-0"
                          onClick={() =>
                        ...
                        >
                          <img
                            ...
                            className="img-fluid m-0 h-100 rounded rounded-3"
                            alt=""
                          />
                          <div className="restro-detail bg-light rounded box-shadow p-2 mx-3">
                            <small className="bg-success rounded px-2">
                              NEW
                            </small>
                            <div
                            ...
                            >
                              <p className="fs-6 fw-bold text-uppercase">
                                {item.name}
                              </p>
                            </div>
                            <small className="text-success text-sm">
                              {`${item.rating}`.slice(0, 3)}/5 ratings
                            </small>
                            <br />
                            <small className="text-sm fw-bold text-secondary">
