...
function App() {
  const isApproved = useSelector(
    (state) => state?.deliver?.deliverInfo?.deliveryBoyInfo?.isApproved
  );
  const isVerified = useSelector(
    (state) => state?.deliver?.deliverInfo?.deliveryBoyInfo?.isVerified
  );
  ...
  const refresh = async () => {
    try {
      const response = await axios.post(
        "https://foodpoint-backend-je3y.onrender.com/api/delivery/refresh",
        {},
