  ...
  const sendData = async () => {
    try {
      const response = await axios.post(
        "https://foodpoint-backend-je3y.onrender.com/api/delivery/signup",
        data
      );
  ...
  return (
    <div className="login-page d-flex">
      <div className="login-container px-5 py-3 border rounded rounded-3">
        <h4 className="fs-2 text-center fw-bold text-secondary">
          Register new Deliver Partner
        </h4>
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" className=" w-100 mt-3 btn btn-outline-danger">
            Sign Up
          </button>
          <small>
